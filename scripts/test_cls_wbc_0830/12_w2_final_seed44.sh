#!/bin/bash
# W2-Final: selected complete configuration, random seed 44.
...
EXPERIMENT_ID="W2-Final"
AUGMENT="true"
AUGMENTATION_COPIES="${SELECTED_AUGMENTATION_COPIES:-2}"
AUGMENTATION_POLICY="${SELECTED_AUGMENTATION_POLICY:-dihedral_perspective}"
PERSPECTIVE_PROBABILITY="${SELECTED_PERSPECTIVE_PROBABILITY:-0.5}"
PERSPECTIVE_SCALE="${SELECTED_PERSPECTIVE_SCALE:-0.05}"
PERSPECTIVE_PADDING_MODE="${SELECTED_PERSPECTIVE_PADDING_MODE:-reflection}"
BALANCE_MODE="${SELECTED_BALANCE_MODE:-none}"  # none | balanced_sampling | inverse_freq_loss
CLASSIFICATION_HEAD="${SELECTED_CLASSIFICATION_HEAD:-h2_dual_scale}"
DEFAULT_LR="${SELECTED_LR:-8e-4}"
SEED=44
...
# Fixed WBC-0830 in-domain p049/p011/p011 split; this script never scans data/ for a newer split.
TRAIN_ROOT="${TRAIN_ROOT:-data/2018WBC_cellcrop_512x512_noresize_contiguous20_entropy_multicandidate_manualoverride_filtered_minmax_20260830_finetune_train_p049_20260901}"
VAL_ROOT="${VAL_ROOT:-data/2018WBC_cellcrop_512x512_noresize_contiguous20_entropy_multicandidate_manualoverride_filtered_minmax_20260830_finetune_val_p011_20260901}"
TEST_ROOT="${TEST_ROOT:-data/2018WBC_cellcrop_512x512_noresize_contiguous20_entropy_multicandidate_manualoverride_filtered_minmax_20260830_finetune_test_p011_20260901}"
...
# Fixed for every core WBC-0830 experiment.
PATCH_SIZE=16
...
# Formal comparisons test only the globally best validation Macro-F1 checkpoint.
...
SAVE_DIR="./records/test_cls_wbc_0830/${RUN_TAG}_${EXP_TIME}"
...
echo "Input/token:             512x512x20 / P${PATCH_SIZE} SP${SPECTRAL_PATCH_SIZE}"

#!/bin/bash
# W2-H1-DIH-C2-BAL: new CellCrop data, H1 head, dihedral copies=2,
# balanced sampling, and the MSE<=0.01 finetune-only split.
set -euo pipefail
...
EXPERIMENT_ID="W2-H1-DIH-C2-BAL-MSE001"
AUGMENT="true"
AUGMENTATION_COPIES="${AUGMENTATION_COPIES:-2}"
AUGMENTATION_POLICY="${AUGMENTATION_POLICY:-dihedral}"
PERSPECTIVE_PROBABILITY="${PERSPECTIVE_PROBABILITY:-0.0}"
PERSPECTIVE_SCALE="${PERSPECTIVE_SCALE:-0.05}"
PERSPECTIVE_PADDING_MODE="${PERSPECTIVE_PADDING_MODE:-reflection}"
BALANCE_MODE="balanced_sampling"  # none | balanced_sampling | inverse_freq_loss
CLASSIFICATION_HEAD="h1_attention_mlp"
DEFAULT_LR="8e-4"
SEED="${SEED:-42}"
...
# Pin this comparison to the MSE<=0.01, finetune-only split generated on
# 2026-09-03. Later split runs must not silently change the experiment protocol.
TRAIN_ROOT="${TRAIN_ROOT:-data/2018WBC_cellcrop_512x512_noresize_contiguous20_entropy_multicandidate_manualoverride_filtered_minmax_20260830_finetune_train_p070_20260903}"
VAL_ROOT="${VAL_ROOT:-data/2018WBC_cellcrop_512x512_noresize_contiguous20_entropy_multicandidate_manualoverride_filtered_minmax_20260830_finetune_val_p015_20260903}"
TEST_ROOT="${TEST_ROOT:-data/2018WBC_cellcrop_512x512_noresize_contiguous20_entropy_multicandidate_manualoverride_filtered_minmax_20260830_finetune_test_p015_20260903}"

...
SCENE_ENDMEMBER_ROOT="${SCENE_ENDMEMBER_ROOT:-data/LUAD_PUAD_official224_centerbalanced_3660/scene_endmembers_K16_l10.0005_l20.0002_l30.01_le0.05_ec3_simplex}"
...
DATASET_INFO="PUAD-official224-centerbalanced3660-tok${PATCH_SIZE}-sp${SPECTRAL_PATCH_SIZE}"
LOSS_MODE_SUFFIX=""
if [ "$SEGMENTATION_LOSS_MODE" = "foreground" ]; then
    LOSS_MODE_SUFFIX="_use-fgDiceLoss"
fi
EXP_TIME=$(date +%Y%m%d_%H%M%S)
SAVE_DIR="${SAVE_DIR:-./records/test_seg_puad_0906/${EXPERIMENT_ID}_${DATASET_INFO}${LOSS_MODE_SUFFIX}_${EXP_TIME}}"
mkdir -p "$SAVE_DIR"

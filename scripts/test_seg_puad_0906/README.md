...
- `data/LUAD_PUAD_official224_centerbalanced_3660/train`
- `data/LUAD_PUAD_official224_centerbalanced_3660/val`
- `data/LUAD_PUAD_official224_centerbalanced_3660/test`

训练/验证输入为 `224×224×40` patch；测试为完整场景，通过 `224` 窗口、
`112` stride和Gaussian融合完成滑窗推理。默认仍使用 `tok16-sp5` 和
...
`2 GPU × 8/GPU = 16`，相对于 `2 GPU × 4/GPU、LR=4e-4` 的参考设置，
按线性缩放默认使用 `LR=8e-4`。可以在命令行覆盖全部顺序实验：
...
SAVE_DIR=records/test_seg_puad_0906/inference_stride56 \
bash scripts/test_seg_puad_0906/13_p_final_seed42.sh
```

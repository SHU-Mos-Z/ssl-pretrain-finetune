#!/bin/bash
# Conditioned HSI detection fine-tuning. Switch both detector and feature path here.
set -euo pipefail
cd "$(dirname "$0")/.."
...
DET_FEATURE_MODE="z_pyramid"    # z_pyramid | gated_pyramid | z_full

# ── 输入视图协议 ──────────────────────────────────────────────────────────────
# direct 保持 MDC/GPCC 的历史行为；runtime_window 用于不可变全场景数据。
DETECTION_VIEW_MODE="direct"    # direct | runtime_window
...
ENABLE_CROP_TRUNCATED_POSITIVE=true
EVAL_OWNERSHIP_FILTER=true
...
# 下列目录来自 2026-08-25 完成的、已修正 XML 纵向坐标的检测数据划分。

# MDC（检测）
TRAIN_ROOT="${TRAIN_ROOT:-data/MDC_detection_patch_1024x1280_overlap_0x0_to_256x256_xml_object_minmax_maskflipped_finetune_train_p070_20260825}"
VAL_ROOT="${VAL_ROOT:-data/MDC_detection_patch_1024x1280_overlap_0x0_to_256x256_xml_object_minmax_maskflipped_finetune_val_p015_20260825}"
TEST_ROOT="${TEST_ROOT:-data/MDC_detection_patch_1024x1280_overlap_0x0_to_256x256_xml_object_minmax_maskflipped_finetune_test_p015_20260825}"
...
EVALUATION_INTERVAL=1     # 每 N 个 epoch 完整验证；首轮和末轮始终验证
PROGRESS="log"
LOG_INTERVAL=10
VIEW_TAG="$DETECTION_VIEW_MODE"
if [ "$DETECTION_VIEW_MODE" = "runtime_window" ]; then
    VIEW_TAG="crop${SOURCE_CROP_SIZE//,/x}-in${MODEL_INPUT_SIZE//,/x}-s${EVAL_STRIDE//,/x}"
fi
DATASET_INFO="$(dataset_info_from_roots "$TRAIN_ROOT" "$VAL_ROOT" "$TEST_ROOT")-maskflipped-${VIEW_TAG}-tok${PATCH_SIZE}-sp${SPECTRAL_PATCH_SIZE}"
EXP_TIME=$(date +%Y%m%d_%H%M%S)
SAVE_DIR="./records/finetune_conditioned_detection/${DATASET_INFO}_${DETECTION_MODE}_${DET_FEATURE_MODE}_${EXP_TIME}"
mkdir -p "$SAVE_DIR"
...
if [ -n "$ANCHOR_CONFIG_JSON" ]; then OPTIONAL_ARGS+=(--anchor-config-json "$ANCHOR_CONFIG_JSON"); fi
if [ "$DETECTION_VIEW_MODE" = "runtime_window" ]; then
    OPTIONAL_ARGS+=(
        --source-crop-size "$SOURCE_CROP_SIZE"
    ...
    if [ "$ENABLE_CROP_TRUNCATED_POSITIVE" = true ]; then
        OPTIONAL_ARGS+=(--enable-crop-truncated-positive)
    else
        OPTIONAL_ARGS+=(--no-enable-crop-truncated-positive)
    fi
    if [ "$EVAL_OWNERSHIP_FILTER" = true ]; then
        OPTIONAL_ARGS+=(--eval-ownership-filter)
    else
        OPTIONAL_ARGS+=(--no-eval-ownership-filter)
    fi
fi

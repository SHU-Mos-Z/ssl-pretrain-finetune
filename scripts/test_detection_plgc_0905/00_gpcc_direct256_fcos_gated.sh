#!/bin/bash
# GPCC/PLGC-IM D1: preprocessed 256x256 view + gated pyramid + FCOS.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# 该数据已经在预处理阶段整体缩放至 256x256，标注也处于同一坐标系。
...
if [ "${DRY_RUN:-0}" = "1" ]; then
    echo "DRY_RUN=1: fixed split paths, annotations, NMF caches and checkpoint passed validation; training was not started."
    exit 0
fi
...
SAVE_DIR="records/test_detection_plgc_0905/GPCC-D1_${DATASET_INFO}_fcos_gated_seed${SEED}_${EXP_TIME}"
mkdir -p "$SAVE_DIR"

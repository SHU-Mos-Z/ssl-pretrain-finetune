#!/bin/bash
# Run the MDC segmentation experiment matrix sequentially on one fixed GPU set.
set -euo pipefail
...
export SEGMENTATION_LOSS_MODE="${SEGMENTATION_LOSS_MODE:-}"
export PRETRAIN_CKPT="${PRETRAIN_CKPT:-records/pretrain_conditioned/20260817_005610/ckpt_last.pth}"
export TRAIN_ROOT="${TRAIN_ROOT:-data/MDC_Resized_256_320_to_256_256_overlap_0_0_preprocessed_finetune_train_p049_20260728}"
export VAL_ROOT="${VAL_ROOT:-data/MDC_Resized_256_320_to_256_256_overlap_0_0_preprocessed_finetune_val_p010_20260728}"
export TEST_ROOT="${TEST_ROOT:-data/MDC_Resized_256_320_to_256_256_overlap_0_0_preprocessed_finetune_test_p010_20260728}"
...
EXPERIMENT_SCRIPTS=(
    "$SCRIPT_DIR/00_m_00_default.sh"
    "$SCRIPT_DIR/01_m_a1_dihedral.sh"
    "$SCRIPT_DIR/02_m_a2_dihedral_affine.sh"
    "$SCRIPT_DIR/03_m_a3_dihedral_perspective.sh"
    "$SCRIPT_DIR/04_m_h1_residual.sh"
    "$SCRIPT_DIR/05_m_h2_aspp.sh"
    "$SCRIPT_DIR/06_m_h3_multiscale_aux.sh"
    "$SCRIPT_DIR/07_m_l1_weighted_ce_dice.sh"
    "$SCRIPT_DIR/08_m_l2_focal_dice.sh"
    "$SCRIPT_DIR/09_m_l3_weighted_boundary.sh"
    "$SCRIPT_DIR/10_m_o1_discriminative_lr.sh"
    "$SCRIPT_DIR/11_m_o2_staged_unfreeze.sh"
    "$SCRIPT_DIR/12_m_final_seed42.sh"
    "$SCRIPT_DIR/13_m_final_seed43.sh"
    "$SCRIPT_DIR/14_m_final_seed44.sh"
)
...
echo "============================================================================"
echo "MDC sequential segmentation experiment runner"
...
echo "LR:                     $LR"
echo "EPOCHS:                 $EPOCHS"
echo "SEGMENTATION_LOSS_MODE: ${SEGMENTATION_LOSS_MODE:-<python default: all_class>}"

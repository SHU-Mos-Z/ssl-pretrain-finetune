...
# Experiment T-O2: hold backbone weights for 20 epochs, then fine-tune.
EXPERIMENT_ID="G-O2"
AUGMENT="true"
...
AUX_LOSS_WEIGHT="${SCREEN_AUX_LOSS_WEIGHT:-0.0}"
SEGMENTATION_LOSS="${SCREEN_SEGMENTATION_LOSS:-ce_dice}"
CLASS_WEIGHT_MODE="${SCREEN_CLASS_WEIGHT_MODE:-none}"
BOUNDARY_LOSS_WEIGHT="${SCREEN_BOUNDARY_LOSS_WEIGHT:-0.0}"
FOCAL_GAMMA="${SCREEN_FOCAL_GAMMA:-2.0}"
...
# GPCC fixed splits (all inputs are 256x256 patches)
TRAIN_ROOT="${TRAIN_ROOT:-data/GPCC_Resized_512_640_to_256_256_overlap_0_0_preprocessed_finetune_train_p070_20260728}"
VAL_ROOT="${VAL_ROOT:-data/GPCC_Resized_512_640_to_256_256_overlap_0_0_preprocessed_finetune_val_p015_20260728}"
TEST_ROOT="${TEST_ROOT:-data/GPCC_Resized_512_640_to_256_256_overlap_0_0_preprocessed_finetune_test_p015_20260728}"
...
DATASET_INFO="GPCC-256x256-b40-tok${PATCH_SIZE}-sp${SPECTRAL_PATCH_SIZE}"
LOSS_MODE_SUFFIX=""
if [ "$SEGMENTATION_LOSS_MODE" = "foreground" ]; then
    LOSS_MODE_SUFFIX="_use-fgDiceLoss"
fi
EXP_TIME=$(date +%Y%m%d_%H%M%S)
SAVE_DIR="${SAVE_DIR:-./records/test_seg_gpcc_0913/${EXPERIMENT_ID}_${DATASET_INFO}${LOSS_MODE_SUFFIX}_${EXP_TIME}}"
mkdir -p "$SAVE_DIR"

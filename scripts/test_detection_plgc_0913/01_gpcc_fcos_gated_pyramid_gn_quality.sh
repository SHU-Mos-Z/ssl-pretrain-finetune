#!/bin/bash
# GPCC 0913 E01: fcos + gated_pyramid + GroupNorm + IoU quality.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
if [ -z "${FCOS_RANGES:-}" ]; then
    FCOS_RANGES="$("$PYTHON_BIN" -c 'import sys; s=float(sys.argv[1])/256.0; print(f"0:{32*s:g},{32*s:g}:{64*s:g},{64*s:g}:{128*s:g},{128*s:g}:100000000")' "$INPUT_HEIGHT")"
fi
DETECTOR_GEOMETRY_ARGS=(
    --fcos-regression-ranges "$FCOS_RANGES"
    --fcos-center-sampling-radius "${FCOS_CENTER_RADIUS:-1.5}"
    --fcos-normalize-reg-targets-by-stride
)


EXP_TIME=$(date +%Y%m%d_%H%M%S)
SAVE_DIR="records/test_detection_plgc_0913/GPCC-E01_${DATASET_INFO}-direct-tok${PATCH_SIZE}-sp${SPECTRAL_PATCH_SIZE}_fcos-gated_pyramid-gn_quality_seed${SEED}_${EXP_TIME}"
mkdir -p "$SAVE_DIR"

#!/bin/bash
# Experiment 5/7: anchor-free gated pyramid, full fine-tuning with a conservative backbone learning rate.
set -euo pipefail
cd "$(dirname "$0")/../.."
...
# ── 核心实验开关：支持全部六种组合 ───────────────────────────────────────────
DETECTION_MODE="anchor_free"   # anchor_based | anchor_free
...
EXPERIMENT_TAG="fullfinetune-lr2e-4-bblrmult0p01"

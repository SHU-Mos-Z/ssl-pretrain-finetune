#!/bin/bash
# W-Final: selected configuration repeated with at least three random seeds.
...
EXPERIMENT_ID="W-Final"
DATA_VARIANT="new"
AUGMENT="true"
SELECTED_AUGMENTATION_COPIES="${SELECTED_AUGMENTATION_COPIES:-2}"
SELECTED_BALANCE_MODE="${SELECTED_BALANCE_MODE:-balanced_sampling}"
BEST_CLASSIFICATION_HEAD="${BEST_CLASSIFICATION_HEAD:-h2_dual_scale}"
AUGMENTATION_COPIES="$SELECTED_AUGMENTATION_COPIES"
BALANCE_MODE="$SELECTED_BALANCE_MODE"
CLASSIFICATION_HEAD="$BEST_CLASSIFICATION_HEAD"
AUGMENT="${AUGMENT:-true}"
AUGMENTATION_COPIES="${AUGMENTATION_COPIES:-1}"
BALANCE_MODE="${BALANCE_MODE:-none}"  # none | balanced_sampling | inverse_freq_loss
CLASSIFICATION_HEAD="${CLASSIFICATION_HEAD:-h0_gap_linear}"
...
# Fixed WBC-0818 split; this script never scans data/ for a newer split.
TRAIN_ROOT="${TRAIN_ROOT:-data/2018WBC_patch_1300x1800_overlap_0x0_to_256x256_minmax_bands50_finetune_train_p070_20260818}"
VAL_ROOT="${VAL_ROOT:-data/2018WBC_patch_1300x1800_overlap_0x0_to_256x256_minmax_bands50_finetune_val_p015_20260818}"
TEST_ROOT="${TEST_ROOT:-data/2018WBC_patch_1300x1800_overlap_0x0_to_256x256_minmax_bands50_finetune_test_p015_20260818}"
...
FINAL_SEEDS="${FINAL_SEEDS:-42 43 44}"
AMP="${AMP:-true}"
...
RUN_TAG="${EXPERIMENT_ID}_${DATASET_INFO}-tok${PATCH_SIZE}-sp${SPECTRAL_PATCH_SIZE}-${CLASSIFICATION_HEAD}-aug${AUGMENTATION_COPIES}-${BALANCE_MODE}-seed${SEED}"
EXP_TIME=$(date +%Y%m%d_%H%M%S)
SAVE_DIR="./records/test_cls_wbc_0818/${RUN_TAG}_${EXP_TIME}"
...
for run_seed in $FINAL_SEEDS; do
